...
const { apiMessage } = require('../services/api-messages.service');
...
function notFoundHandler(req, res) { res.status(404).json({ error: { code: 'NOT_FOUND', message: apiMessage(req.locale, 'NOT_FOUND', 'Route was not found.') } }); }
function errorHandler(error, req, res, next) { // eslint-disable-line no-unused-vars
  ...
  if (error instanceof ZodError) return res.status(422).json({ error: { code: 'VALIDATION_ERROR', message: apiMessage(req.locale, 'VALIDATION_ERROR', 'One or more fields are invalid.'), details: error.flatten(), requestId } });
  if (error instanceof AppError) return res.status(error.status).json({ error: { code: error.code, message: apiMessage(req.locale, error.code, error.message), details: error.details, requestId } });
  ...
  logError(error, requestId);
  return res.status(500).json({ error: { code: 'INTERNAL_ERROR', message: apiMessage(req.locale, 'INTERNAL_ERROR', 'An unexpected server error occurred.'), requestId } });
}
